...
public:
    ...
    void setBuildingOccupy(Building* building, bool occupy);
private:
    // --- 场景内部变量 ---
    ...
    // 检查建筑当前位置是否合法
    bool isBuildingPositionValid(Building* building);

    // 获取建筑占用的所有网格坐标
    std::vector<Vec2> getBuildingOccupiedTiles(Building* building);
