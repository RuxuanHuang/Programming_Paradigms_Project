...
void HelloWorld::displayPlayerResources()
{
    ...
    // 金币数量
    auto goldLabel = Label::createWithTTF(resourceMgr->getGoldString(),
        "fonts/arial.ttf", 28);
    ...
    // 水晶数量
    auto crystalLabel = Label::createWithTTF(resourceMgr->getCrystalString(),
        "fonts/arial.ttf", 28);
...
void BeachScene::updateResourceDisplay()
{
    ...
    if (goldLabel) {
        goldLabel->setString(resourceMgr->getGoldString());
    }

    // 更新水晶显示
    auto crystalLabel = (Label*)this->getChildByTag(CRYSTAL_LABEL_TAG);
    if (crystalLabel) {
        crystalLabel->setString(resourceMgr->getCrystalString());
    }
}
    ...
    for (int i = 0; i < visibleImages.size(); i++) {
        ...
        if (sprite && sprite->getBoundingBox().containsPoint(touchPos)) {
            ...
            if (actualIndex < allImages.size()) {
                ...
                auto resourceMgr = ResourceManager::getInstance();
                // 使用资源管理器检查是否可以购买
                if (resourceMgr->canAffordPurchase(goldCost, crystalCost)) {
                    // 使用资源管理器执行购买，不允许购买后得零
                    if (resourceMgr->makePurchase(goldCost, crystalCost, false)) {
                        // 保存购买成功的图片信息
                        ...
                        auto failLabel = Label::createWithTTF("购买失败！", "fonts/arial.ttf", 20);
                        failLabel->setPosition(Vec2(visibleSize.width / 2.0f, 400.0f));
                        failLabel->setColor(Color3B::RED);
                        this->addChild(failLabel, 6);

                        failLabel->runAction(Sequence::create(
